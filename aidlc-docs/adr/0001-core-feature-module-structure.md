# ADR-0001: core/feature Gradle 모듈 구조 채택

- Status: accepted (2026-08-31, 사용자 결정: 디자인 시스템 충족 후 core/feature 단위 모듈 분리로 전환)
- Context: 기존 원칙은 "단일 shared 모듈 + feature-first 패키지, 멀티모듈은 독립 소비자 발생 시" (kmp-module-structure 규칙 + project-profile). :core:designsystem은 catalogApp 소비자로 이미 모듈. 로그인 화면 배치 논의에서 사용자가 core/feature 단위 멀티모듈 구조로 전환을 확정.
- Options:
  - A) 단일 shared 유지 (기존 규칙) — 모듈 오버헤드 없음, 그러나 사용자가 원하는 경계 명시성 부족
  - B) core/feature 모듈 분리 — 경계가 Gradle로 강제됨, 빌드 구성 비용 증가
- Decision: B. 구조 규칙:
